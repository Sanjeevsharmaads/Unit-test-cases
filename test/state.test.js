...
describe('#state', () => {
  ...
  describe('(key, value)', () => {
    it('passes when the actual matches the expected 4', (wrapper) => {
      expect(wrapper).to.have.state('foo', 'bar')
    }, { render: false })

    it('passes negated when the actual does not match the expected 5', (wrapper) => {
      expect(wrapper).to.not.have.state('foo', 'baz')
    }, { render: false })

    it('fails when the actual does not match the expected 6', (wrapper) => {
      expect(() => {
  ...
  it('fails when the actual is undefined 7', () => {
    expect(() => {
